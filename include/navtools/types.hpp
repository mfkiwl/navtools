...
#define ASSERT_EIGEN_NUM_ROWS(T, obj, Rows)                                      \
  if constexpr (T::RowsAtCompileTime != Eigen::Dynamic) {                        \
    static_assert(T::RowsAtCompileTime == Rows, "Inappropriate number of rows"); \
  } else {                                                                       \
...
#define ASSERT_EIGEN_NUM_COLS(T, obj, Cols)                                         \
  if constexpr (T::ColsAtCompileTime != Eigen::Dynamic) {                           \
    static_assert(T::ColsAtCompileTime == Cols, "Inappropriate number of columns"); \
  } else {                                                                          \
...
#define ASSERT_EIGEN_SCALAR_TYPE(Derived, T) \
  static_assert(                             \
      std::is_same_v<typename Derived::Scalar, T>, "Eigen scalar type constraint not upheld")
